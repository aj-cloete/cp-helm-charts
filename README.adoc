...
The https://github.com/aj-cloete/cp-helm-charts[Confluent Platform Helm Charts] enable you to deploy Confluent Platform components on Kubernetes for development, test, and proof of concept environments.
...
----
helm repo add ajcloete https://aj-cloete.github.io/cp-helm-charts   #<1>
helm repo update    #<2>
helm install ajcloete/cp-helm-charts --name my-confluent --version 0.6.0    #<3>
----
<1> Add `ajcloete` helm charts repo
...
* Report all enhancements, bugs, and tasks as https://github.com/aj-cloete/cp-helm-charts/issues[GitHub issues]
...
You can follow up on Helm 3 issues in https://github.com/aj-cloete/cp-helm-charts/issues/480
...
You can define the size of the volumes by changing `dataDirSize` and `dataLogDirSize` under `cp-zookeeper` and `size` under `cp-kafka` in https://github.com/aj-cloete/cp-helm-charts/blob/master/values.yaml[values.yaml].
...
The `StorageClass` that was created can be specified in `dataLogDirStorageClass` and `dataDirStorageClass` under `cp-zookeeper` and in `storageClass+` under `cp-kafka` in https://github.com/aj-cloete/cp-helm-charts/blob/master/values.yaml[values.yaml].

To deploy non-persistent Kafka and ZooKeeper clusters, you must change  the value of `persistence.enabled` under `cp-kafka` and `cp-zookeeper` in https://github.com/aj-cloete/cp-helm-charts/blob/master/values.yaml[values.yaml]
...
----
git clone https://github.com/aj-cloete/cp-helm-charts.git        #<1>
...
Now that you have running in your Kubernetes cluster, you may run a https://github.com/aj-cloete/cp-helm-charts/blob/master/examples/ksql-demo.yaml[ksqlDB example].
...
. Import dashboard under https://github.com/aj-cloete/cp-helm-charts/blob/master/grafana-dashboard/confluent-open-source-grafana-dashboard.json[grafana-dashboard] into Grafana image:screenshots/kafka.png[Kafka Dashboard]
